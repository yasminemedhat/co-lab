...
homepage.get('/', auth, async (req, res) => {
    ...
    var lastSeen = req.body.lastSeen; //get last seen post from frontend 
    ...
    try {
       ...
       //construct match query to avoid having 2 separate queries
        if(lastSeen != null){ //in case this isn't the user's first request
            var matchQuery={
                $and: [
                    {
        ...
        await Colaber.populate(posts,
            { path: 'creator', select: { firstName: 1, lastName: 1, email: 1, _id: 1, avatar: 1 } }
        );
        if (!posts)
